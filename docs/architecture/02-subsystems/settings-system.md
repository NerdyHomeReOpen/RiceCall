...
```mermaid
graph LR
    UI[SystemSetting.tsx] -- "ipc.systemSettings.set" --> Facade[src/ipc.ts]
    Facade -- Handler --> Logic[src/platform/ipc/handlers/shared/settings.ts]
    Logic -- Storage --> LocalStore[Electron Store / LocalStorage]
    
    UI -- "ipc.socket.send" --> Facade
    Facade -- "socket.emit" --> Server[RiceCall Server]
```
...
### 3.1 Handler (`src/platform/ipc/handlers/shared/settings.ts`)
...
2.  **設定預設值**: 在 `src/platform/ipc/handlers/shared/settings.ts` 的 `DEFAULTS` 物件中新增該欄位與預設值。
3.  **註冊處理器**: 在 `src/platform/ipc/handlers/shared/settings.ts` 的 `settingKeys` 陣列中加入該欄位名稱。Handler 會自動產生對應的 IPC 通道。
