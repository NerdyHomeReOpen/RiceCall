...
interface ChannelTabProps {
  ...
  channel: Channel;
  permissionLevelOverride?: number;
...
const ChannelTab: React.FC<ChannelTabProps> = React.memo(
  ({ user, friends, server, serverOnlineMembers, channel, expanded, selectedItemId, permissionLevelOverride, setExpanded, setSelectedItemId }) => {
    // Hooks
    ...
    const permissionLevel = useMemo(
      () => Math.max(globalPermissionLevel, serverPermissionLevel, channelPermissionLevel, permissionLevelOverride || 0),
      [globalPermissionLevel, serverPermissionLevel, channelPermissionLevel, permissionLevelOverride],
    );
