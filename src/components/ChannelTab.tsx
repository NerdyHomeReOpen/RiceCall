...
const ChannelTab: React.FC<ChannelTabProps> = ({
  ...
  const filteredChannelMembers = useMemo(
    () => (!isExpanded ? [] : channelMembers.filter(Boolean).sort((a, b) => b.lastJoinChannelAt - a.lastJoinChannelAt || (a.nickname || a.name).localeCompare(b.nickname || b.name))),
...
const areChannelTabPropsEqual = (prev: Readonly<ChannelTabProps>, next: Readonly<ChannelTabProps>) => {
  ...
  if (prev.currentServer !== next.currentServer) return false;
...
export default React.memo(ChannelTab, areChannelTabPropsEqual);
