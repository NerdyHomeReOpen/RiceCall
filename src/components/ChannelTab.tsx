...
const ChannelTab: React.FC<ChannelTabProps> = React.memo(
  ({ user, currentServer, currentChannel, friends, queueUsers, serverOnlineMembers, channel, expanded = { [channel.channelId]: true }, selectedItemId, setExpanded = () => {}, setSelectedItemId }) => {
    // Hooks
    ...
    const channelMembers = serverOnlineMembers.filter((m) => m.currentChannelId === channelId);
    const movableServerUserIds = serverOnlineMembers.filter((m) => m.userId !== userId && m.permissionLevel <= permissionLevel).map((m) => m.userId);
    const movableChannelUserIds = channelMembers.filter((m) => m.userId !== userId && m.permissionLevel <= permissionLevel).map((m) => m.userId);
