...
interface UseSFUTransportDeps {
  ...
  initSpeakerAudio: (userId: string, stream: MediaStream) => void;
  removeSpeakerAudio: (userId: string) => void;
  startSpeaking: () => void;
  stopSpeaking: () => void;
}

export const useSFUTransport = (refs: SharedRefs, { initAudioContext, initSpeakerAudio, removeSpeakerAudio, startSpeaking, stopSpeaking }: UseSFUTransportDeps) => {
  ...
  const setupSend = useCallback(
    async (channelId: string) => {
      if (!inputDesRef.current) {
        initAudioContext().then(() => setupSend(channelId));
        return;
      }

      const track = inputDesRef.current.stream.getAudioTracks()[0];
      if (!track) {
        new Logger('WebRTC').error('setupSend: no input track available');
        return;
      }

      if (sendTransportRef.current) {
        const old = sendTransportRef.current;
      ...
      sendTransport.on('connectionstatechange', async (s) => {
        ...
        if (s === 'failed' || s === 'disconnected' || s === 'closed') {
          ...
          if (currentChannelIdRef.current && sendRetryCountRef.current < MAX_TRANSPORT_RETRY_COUNT) {
    ...
    [audioProducerRef, inputDesRef, bitrateRef, deviceRef, sendTransportRef, initAudioContext],
  );

  const setupRecv = useCallback(
    async (channelId: string) => {
      if (recvTransportRef.current) {
        const old = recvTransportRef.current;
      ...
      const transport = await ipc.socket.emit('SFUCreateTransport', { direction: 'recv', channelId }).catch((e) => {
      ...
      recvTransport.on('connectionstatechange', (s) => {
        ...
        if (s === 'failed' || s === 'disconnected' || s === 'closed') {
          if (currentChannelIdRef.current && recvRetryCountRef.current < MAX_TRANSPORT_RETRY_COUNT) {
