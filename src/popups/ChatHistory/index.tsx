                  ...
                  <div className={styles['message-section']}>
                    {messages.map((msg) => (
                      <div key={msg.id} className={`${styles['message-item']} ${selectedMsg === msg.id ? styles['selected'] : ''}`} onClick={() => setSelectedMsg(msg.id)}>
                        ...
                        <div className={styles['message-text']}>{msg.message_text}</div>
                      </div>
                    ))}
                  </div>
                </div>
                <div className={styles['pagination-wrapper']}>
                  <span className={styles['page-index']}>
                  ...
                  {[...Array(totalPages)].map((_, idx) => {
                    const page = idx + 1;
                    return (
                      <label key={page} className={`${styles['radio-wrapper']} ${currentPage === page ? styles['selected'] : ''}`}>
